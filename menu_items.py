...
def transform_menu_items(source_data, target_data, source_ctx, target_ctx):
    ...
    #print("{0}".format(etl.header(source_data)))
    ...
    category_map["category_id"] = "category_id", \
        lambda value: transformed_categories[value]

    categories_cat_ids = etl.fieldmap(menu_items, category_map)
    #print(table1.lookall())

    # @TODO - put this into a method for cleanliness
    # if mmj product genetics field contains any of the g1 category names.
    transformed_genetics = {}
    genetics = etl.facet(menu_items, 'genetics')
    for genetic in genetics.keys():
        if genetic and genetic in target_category_names:
            target_cat_id = target_ctx.collection.find_one(
                {'name': genetic})
            split_rows = etl.select(menu_items, 'genetics', 
                lambda value: value == genetic)
            filtered_genetics = split_rows.values('genetics')
            for filtered_genetic in filtered_genetics:
                transformed_genetics[filtered_genetic] = target_cat_id.get('_id')

    genetics_map = OrderedDict()
    genetics_map["category_id"] = "genetics", \
        lambda value: transformed_genetics[value]

    genetics_cat_ids = etl.fieldmap(menu_items, genetics_map)

    # cat = etl.cat(table1, table2)
    # print(cat.lookall())
